...
namespace System.Web.OData.NHibernate
{
    public class NHibernateFilterBinder
    {
        ...
        public NHibernateFilterBinder(IEdmModel model)
        {
            _model = model;
        }
        ...
        public static WhereClause GetWhere(FilterClause filterQuery,IEdmModel model)
        {
            if (filterQuery != null)
            {
                ...
                return new WhereClause
                {
                    Clause = "where " + binder.BindFilterClause(filterQuery) + Environment.NewLine,
                    PositionalParameters = binder._positionalParmeters.ToArray()
                };
            }
            ...
            else if (singleValueNode != null)
            {
                switch (node.Kind)
                {
                    ...
                    case QueryNodeKind.ResourceRangeVariableReference:
                       // return BindRangeVariable(node as ResourceRangeVariable);
                        return null;
